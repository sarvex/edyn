#include "edyn/util/aabb_util.hpp"
#include "edyn/util/shape_util.hpp"
#include "edyn/math/transform.hpp"
#include "edyn/math/matrix2x2.hpp"
#include <variant>

namespace edyn {

static constexpr scalar aabb_half_extent = 99999;

AABB plane_aabb(const vector2 &normal, scalar constant) {
    vector2 unit_min, unit_max;

    if (normal == vector2_x) {
        unit_min = {-1, -1};
        unit_max = { 0,  1};
    } else if (normal == -vector2_x) {
        unit_min = { 0, -1};
        unit_max = { 1,  1};
    } else if (normal == vector2_y) {
        unit_min = {-1, -1};
        unit_max = { 1,  0};
    } else if (normal == -vector2_y) {
        unit_min = {-1,  0};
        unit_max = { 1,  1};
    } else {
        unit_min = {-1, -1};
        unit_max = { 1,  1};
    }

    auto pos_world = normal * constant;
    return {unit_min * aabb_half_extent + pos_world,
            unit_max * aabb_half_extent + pos_world};
}

AABB box_aabb(const vector2 &half_extents, const vector2 &pos, scalar orn) {
    // Reference: Real-Time Collision Detection - Christer Ericson, section 4.2.6.
    auto aabb = AABB{pos, pos};
    auto basis = rotation_matrix2x2(orn);

    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            auto e = basis[i][j] * -half_extents[j];
            auto f = -e;

            if (e < f) {
                aabb.min[i] += e;
                aabb.max[i] += f;
            } else {
                aabb.min[i] += f;
                aabb.max[i] += e;
            }
        }
    }

    return aabb;
}

AABB circle_aabb(scalar radius, const vector2 &pos) {
    return {
        {pos.x - radius, pos.y - radius},
        {pos.x + radius, pos.y + radius}
    };
}

AABB capsule_aabb(scalar radius, scalar half_length, const vector2 &pos, scalar orn) {
    auto m = rotation_matrix2x2(orn);
    auto v = m * vector2_x * half_length;
    auto p0 = pos - v;
    auto p1 = pos + v;
    auto offset = vector2 {radius, radius};
    return {min(p0, p1) - offset, max(p0, p1) + offset};
}

AABB aabb_to_world_space(const AABB &aabb, const vector2 &pos, scalar orn) {
    auto center_local = (aabb.min + aabb.max) * scalar(0.5);
    auto center = to_world_space(center_local, pos, orn);
    auto extents = aabb.max - aabb.min;
    auto half_extents = extents * scalar(0.5);

    // Compute support points of the global axes.
    auto c_orn = conjugate(orn);
    auto axis_x = rotate(c_orn, vector2_x);
    auto axis_y = rotate(c_orn, vector2_y);
    auto axis_z = rotate(c_orn, vector2_z);

    auto sup_x = support_point_box(half_extents, axis_x);
    auto sup_y = support_point_box(half_extents, axis_y);
    auto sup_z = support_point_box(half_extents, axis_z);

    auto result_half_extents = vector2{
        dot(sup_x, axis_x),
        dot(sup_y, axis_y),
        dot(sup_z, axis_z)
    };
    auto result_min = center - result_half_extents;
    auto result_max = center + result_half_extents;
    return {result_min, result_max};
}

AABB aabb_to_object_space(const AABB &aabb, const vector2 &pos, scalar orn) {
    auto center_local = (aabb.min + aabb.max) * scalar(0.5);
    auto center = to_object_space(center_local, pos, orn);
    auto extents = aabb.max - aabb.min;
    auto half_extents = extents * scalar(0.5);

    // Compute support points on local axes.
    auto axis_x = rotate(orn, vector2_x);
    auto axis_y = rotate(orn, vector2_y);
    auto axis_z = rotate(orn, vector2_z);

    auto sup_x = support_point_box(half_extents, axis_x);
    auto sup_y = support_point_box(half_extents, axis_y);
    auto sup_z = support_point_box(half_extents, axis_z);

    auto result_half_extents = vector2{
        dot(sup_x, axis_x),
        dot(sup_y, axis_y),
        dot(sup_z, axis_z)
    };
    auto result_min = center - result_half_extents;
    auto result_max = center + result_half_extents;
    return {result_min, result_max};
}

AABB point_cloud_aabb(const std::vector<vector2> &points) {
    // TODO: implement and use `parallel_reduce`.
    auto aabb = AABB{vector2_max, -vector2_max};

    for (auto &point : points) {
        aabb.min = min(aabb.min, point);
        aabb.max = max(aabb.max, point);
    }

    return aabb;
}

AABB point_cloud_aabb(const std::vector<vector2> &points,
                      const vector2 &pos, scalar orn) {
    // TODO: implement and use `parallel_reduce`.
    auto aabb = AABB{vector2_max, -vector2_max};
    auto m = rotation_matrix2x2(orn);

    for (auto &point_local : points) {
        auto point_world = to_world_space(point_local, pos, m);
        aabb.min = min(aabb.min, point_world);
        aabb.max = max(aabb.max, point_world);
    }

    return aabb;
}

AABB shape_aabb(const plane_shape &sh, const vector2 &pos, scalar orn) {
    // Position and orientation are ignored for planes.
    return plane_aabb(sh.normal, sh.constant);
}

AABB shape_aabb(const circle_shape &sh, const vector2 &pos, scalar orn) {
    return circle_aabb(sh.radius, pos);
}

AABB shape_aabb(const capsule_shape &sh, const vector2 &pos, scalar orn) {
    return capsule_aabb(sh.radius, sh.half_length, pos, orn);
}

AABB shape_aabb(const mesh_shape &sh, const vector2 &pos, scalar orn) {
    return {
        sh.trimesh->get_aabb().min + pos,
        sh.trimesh->get_aabb().max + pos
    };
}

AABB shape_aabb(const box_shape &sh, const vector2 &pos, scalar orn) {
    return box_aabb(sh.half_extents, pos, orn);
}

AABB shape_aabb(const polyhedron_shape &sh, const vector2 &pos, scalar orn) {
    return point_cloud_aabb(sh.mesh->vertices, pos, orn);
}

AABB shape_aabb(const paged_mesh_shape &sh, const vector2 &pos, scalar orn) {
    return {
        sh.trimesh->get_aabb().min + pos,
        sh.trimesh->get_aabb().max + pos
    };
}

AABB shape_aabb(const compound_shape &sh, const vector2 &pos, scalar orn) {
    // Using AABB of transformed AABB for greater performance.
    auto aabb = aabb_to_world_space(sh.nodes.front().aabb, pos, orn);

    for (size_t i = 1; i < sh.nodes.size(); ++i) {
        auto aabb_i = aabb_to_world_space(sh.nodes[i].aabb, pos, orn);
        aabb = enclosing_aabb(aabb, aabb_i);
    }

    return aabb;
}

AABB shape_aabb(const shapes_variant_t &var, const vector2 &pos, scalar orn) {
    AABB aabb;
    std::visit([&] (auto &&shape) {
        aabb = shape_aabb(shape, pos, orn);
    }, var);
    return aabb;
}

}
